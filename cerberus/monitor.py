...
<<<<<<< HEAD
        ...
        # and is silent) -- it makes the *behaviour* a visible, frozen catch.
        self._clone_count = 0
        # Rate-based fork-bomb detection: more than `_fork_burst` process spawns
        # within `_fork_window` seconds is a fork bomb. Tuned to sit well below
        # any sane parallel program's startup burst but far under a fork bomb's
        # thousands-per-second, and below a typical pids cap so it fires even
        # when the cap would otherwise stop (and hide) the bomb.
...
<<<<<<< HEAD
        ...
        # cap stops it and it exits -- so an absolute-count threshold above the
        # cap would never fire and the run would look "clean" even though it was
        # hammering the limit. A rate window catches both cases: the cap-enforced
        # burst and the uncapped runaway. No legitimate program forks this fast.
        if sname in self._CLONE_SYSCALLS:
            now = time.monotonic()
            self._clone_times.append(now)
            while self._clone_times and now - self._clone_times[0] > self._fork_window:
                self._clone_times.popleft()
            self._clone_count += 1
            if len(self._clone_times) > self._fork_burst:
                self.stats.notifications += 1
                self.stats.by_syscall[sname] += 1
                span = max(now - self._clone_times[0], 1e-6)
                rate = len(self._clone_times) / span
                v = Verdict(
                    Action.DENY, Severity.VIOLATION, "process.fork_bomb",
                    f"fork bomb — {self._clone_count} process spawns, "
                    f"~{rate:.0f}/s; frozen before it could exhaust the machine",
                    {"spawns": self._clone_count, "rate_per_s": round(rate),
                     "burst_limit": self._fork_burst},
                )
                self.stats.by_rule[v.rule] += 1
