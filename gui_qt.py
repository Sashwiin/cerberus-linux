...
# ---- palette (mirrors the design language of the rest of the project) ------
BG = "#0a0e14"
PANEL = "#111722"
PANEL2 = "#0d131c"
LINE = "#1e2836"
...
APP_STYLESHEET = f"""
QMainWindow, QWidget {{ background: {BG}; color: {INK}; font-family: "DejaVu Sans"; font-size: 10pt; }}
...
QPushButton#danger:disabled {{ background: #3a2020; color: {FAINT}; }}
QCheckBox {{ color: {INK}; }}
...
QListWidget::item:selected {{ background: {PANEL2}; color: {INK}; }}
...
QScrollBar::handle:vertical {{ background: {LINE}; border-radius: 5px; min-height: 20px; }}
"""
...
class CerberusMainWindow(QtWidgets.QMainWindow):
    ...
    def _build_header(self) -> QtWidgets.QWidget:
        ...
        title.setStyleSheet(f"color:{INK}; font-size:14pt; font-weight:700; background:transparent;")
    ...
    def _build_analyze_tab(self) -> QtWidgets.QWidget:
        ...
        self._paint_orb("#2a3646")
        sf_lay.addWidget(self.orb)
        tf = QtWidgets.QVBoxLayout()
        self.state_big = QtWidgets.QLabel("idle")
        self.state_big.setStyleSheet(f"color:{INK}; font-size:16pt; font-weight:700; border:none;")
        ...
        self.banner.setStyleSheet(f"QFrame {{ background:#1a0e12; border:1px solid {BAD}; border-radius:10px; }}")
        ...
        self.banner_lat.setStyleSheet(f"color:white; font-family:{MONO_FAMILY}; font-size:20pt; font-weight:700; border:none;")
    ...
    def _set_state(self, cls: str, big: str, sub: str) -> None:
        colors = {"idle": "#2a3646", "running": INFO, "frozen": BAD, "clean": OK, "error": BAD}
        border = {"idle": LINE, "running": INFO, "frozen": BAD, "clean": OK, "error": BAD}
        self._paint_orb(colors.get(cls, "#2a3646"))
        ...
        self.state_big.setStyleSheet(f"color:{big_color}; font-size:16pt; font-weight:700; border:none;")
    ...
    def _show_error(self, msg: str) -> None:
        self.running = False
        self.banner.setStyleSheet(f"QFrame {{ background:#1a1408; border:1px solid {WARN}; border-radius:10px; }}")
    ...
    def _build_timeline_tab(self) -> QtWidgets.QWidget:
        ...
        head.setStyleSheet(f"color:{INK}; font-size:18pt; font-weight:700;")
        lay.addWidget(head)
